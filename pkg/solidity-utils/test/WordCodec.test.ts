    ...
    describe('encode', () => {
        describe('unsigned', () => {
            ...
            async function assertUnsignedEncoding(value: BigNumberish, offset: number, bits: number) {
                ...
                expect(
                    negate(((BigInt(1) << BigInt(bits)) - BigInt(1)) << BigInt(offset)) & BigInt(result)
                ).to.equal(0);
            }
            ...
            for (const offset of [0, 50, 150, 254]) {
                ...
                context(`with offset ${offset}`, () => {
                    ...
                    it('reverts with large values', async () => {
                        for (let bits = 2; bits <= MAX_BITS; bits++) {
                            await expect(assertUnsignedEncoding(getMaxUnsigned(bits) + bn(1), offset, bits)).to.be.revertedWithCustomError(
                                lib,
                                'CodecOverflow'
                            );
                        }
                    ...
                    it('reverts with large bitsize', async () => {
                        await expect(assertUnsignedEncoding(0, offset, MAX_BITS + 1)).to.be.revertedWithCustomError(lib, 'OutOfBounds');
                    });
                });
            }
        });
            ...
            for (const offset of [0, 50, 150, 254]) {
                ...
                context(`with offset ${offset}`, () => {
                    ...
                    it('reverts with large positive values', async () => {
                        for (let bits = 2; bits <= MAX_BITS; bits++) {
                            await expect(assertSignedEncoding(getMaxSigned(bits) + bn(1), offset, bits)).to.be.revertedWithCustomError(
                                lib,
                                'CodecOverflow'
                            );
                        }
                    ...
                    it('reverts with large negative values', async () => {
                        for (let bits = 2; bits <= MAX_BITS; bits++) {
                            await expect(assertSignedEncoding(getMinSigned(bits) - bn(1), offset, bits)).to.be.revertedWithCustomError(
                                lib,
                                'CodecOverflow'
                            );
                        }
                    ...
                    it('reverts with large bitsize', async () => {
                        await expect(assertSignedEncoding(0, offset, MAX_BITS + 1)).to.be.revertedWithCustomError(lib, 'OutOfBounds');
                    });
                });
            }
        });
        ...
        describe('unsigned', () => {
            ...
            for (const offset of [0, 50, 150, 254]) {
                ...
                context(`with offset ${offset}`, () => {
                    ...
                    it('reverts with large values', async () => {
                        for (let bits = 2; bits <= MAX_BITS; bits++) {
                            await expect(assertUnsignedInsertion(getMaxUnsigned(bits) + bn(1), offset, bits)).to.be.revertedWithCustomError(
                                lib,
                                'CodecOverflow'
                            );
                        }
                    ...
                    it('reverts with large bitsize', async () => {
                        await expect(assertUnsignedInsertion(0, offset, MAX_BITS + 1)).to.be.revertedWithCustomError(lib, 'OutOfBounds');
                    });
                });
            }
        });
        ...
        describe('signed', () => {
            ...
            for (const offset of [0, 50, 150, 254]) {
                ...
                context(`with offset ${offset}`, () => {
                    ...
                    it('reverts with large positive values', async () => {
                        for (let bits = 2; bits <= MAX_BITS; bits++) {
                            await expect(assertSignedInsertion(getMaxSigned(bits) + bn(1), offset, bits)).to.be.revertedWithCustomError(
                                lib,
                                'CodecOverflow'
                            );
                        }
                    ...
                    it('reverts with large negative values', async () => {
                        for (let bits = 2; bits <= MAX_BITS; bits++) {
                            await expect(assertSignedInsertion(getMinSigned(bits) - bn(1), offset, bits)).to.be.revertedWithCustomError(
                                lib,
                                'CodecOverflow'
                            );
                        }
                    ...
                    it('reverts with large bitsize', async () => {
                        await expect(assertSignedInsertion(0, offset, MAX_BITS + 1)).to.be.revertedWithCustomError(lib, 'OutOfBounds');
                    });
                });
            }
        });
