...
library WordCodec {
    ...
    error WordCodecOffsetOutOfBounds(uint256 offset);
    error WordCodecBitLengthOutOfBounds(uint256 bitLength);
    error WordCodecOverflow();
    ...
    function _validateEncodingParams(
        ...
        if (offset >= 256) {
            revert WordCodecOffsetOutOfBounds(offset);
        }
        ...
        if (bitLength == 0 || bitLength > Math.min(255, 256 - offset)) {
            revert WordCodecBitLengthOutOfBounds(bitLength);
        }

        // Testing unsigned values for size is straightforward: their upper bits must be cleared.
        if (value >> bitLength != 0) {
            revert WordCodecOverflow();
        }
    }
