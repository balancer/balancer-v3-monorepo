 ...
 * decimals are meant to be fuzzed by the deriving contract. Pools are created inside the test body with freshly
 * deployed tokens, so the real decimal scaling path in the Vault is exercised.
 *
