...
/**
 * @notice Stable Pool factory that deploys permanently unpauseable pools.
 * @dev This factory is identical to `StablePoolFactory` except that it hardcodes the pause window duration to zero.
 * With a zero-duration factory pause window, `getNewPoolPauseWindowEndTime()` always resolves to 0, so every pool
 * registered by this factory has `pauseWindowEndTime == 0`. The Vault will then revert any pause attempt with
 * `PoolPauseWindowExpired`, meaning pools created here can never be paused.
 *
 * As with `StablePoolFactory`, this allows up to `StableMath.MAX_STABLE_TOKENS` (5) tokens. Since this limit is less
 * than Vault's maximum of 8 tokens, we need to enforce this at the factory level.
 */
contract StablePoolNoPauseFactory is IPoolVersion, BasePoolFactory, Version {
    ...
    /**
     * @notice Deploys a new unpauseable `StablePool`.
