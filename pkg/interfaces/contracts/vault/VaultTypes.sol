...
import { SwapKind, TokenType, TokenConfig } from "../solidity-utils/BasePoolTypes.sol";
...
// There are two potentially confusable "SwapParams" structs defined in the system.
// `VaultSwapParams` - defined here - corresponds to the external swap API defined in the Router contracts, which uses
// explicit token addresses, the amount given and limit on the calculated amount expressed in native token decimals,
// and optional user data passed in from the caller.
//
// `PoolSwapParams` - defined in solidity-utils/BasePoolTypes - passes some of this information through (kind,
// userData), but "translates" the parameters to fit the internal swap API used by `IBasePool`. It scales amounts to
// full 18-decimal precision, adds the token balances, converts the raw token addresses to indices, and adds the
// address of the router originating the request. It does not need the limit, since this is checked at the
// Router level.
