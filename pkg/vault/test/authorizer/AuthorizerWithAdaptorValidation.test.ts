...
describe('AuthorizerWithAdaptorValidation', () => {
  ...
  sharedBeforeEach('deploy actual authorizer and helper', async () => {
    actualAuthorizer = await deploy('v3-solidity-utils/MockBasicAuthorizer', { from: admin });

    vault = await deploy('Vault', { args: [actualAuthorizer.address, ZERO_ADDRESS, 0, 0] });

    authorizerAdaptor = await deploy('v3-liquidity-mining/AuthorizerAdaptor', {
      args: [vault.address],
    });

    adaptorEntrypoint = await deploy('v3-liquidity-mining/AuthorizerAdaptorEntrypoint', {
