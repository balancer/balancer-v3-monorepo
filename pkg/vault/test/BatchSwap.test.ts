...
describe('BatchSwap', function () {
  ...
  sharedBeforeEach('deploy vault, tokens, and pools', async function () {
    ...
    poolA = await deploy('v3-vault/PoolMock', {
      args: [vaultAddress, 'Pool A', 'POOLA', buildTokenConfig(poolATokens), DEFAULT_ROLE_ACCOUNTS, true, 0],
    });

    // Pool A has tokens 1 and 2.
    poolB = await deploy('v3-vault/PoolMock', {
      args: [vaultAddress, 'Pool B', 'POOLB', buildTokenConfig(poolBTokens), DEFAULT_ROLE_ACCOUNTS, true, 0],
    });

    // Pool C has tokens 0 and 2.
    poolC = await deploy('v3-vault/PoolMock', {
      args: [vaultAddress, 'Pool C', 'POOLC', buildTokenConfig(poolCTokens), DEFAULT_ROLE_ACCOUNTS, true, 0],
    });
  });
  ...
  sharedBeforeEach('nested pools', async () => {
    ...
    poolAB = await deploy('v3-vault/PoolMock', {
      args: [vaultAddress, 'Pool A-B', 'POOL-AB', buildTokenConfig(poolABTokens), DEFAULT_ROLE_ACCOUNTS, true, 0],
    });

    poolACTokens = sortAddresses([await poolA.getAddress(), await poolC.getAddress()]);
    poolAC = await deploy('v3-vault/PoolMock', {
      args: [vaultAddress, 'Pool A-C', 'POOL-AC', buildTokenConfig(poolACTokens), DEFAULT_ROLE_ACCOUNTS, true, 0],
    });

    poolBCTokens = sortAddresses([await poolB.getAddress(), await poolC.getAddress()]);
    poolBC = await deploy('v3-vault/PoolMock', {
      args: [vaultAddress, 'Pool B-C', 'POOL-BC', buildTokenConfig(poolBCTokens), DEFAULT_ROLE_ACCOUNTS, true, 0],
    });
  });
