...
contract LiquidityApproximationTest is BaseVaultTest {
    ...
    function createPool() internal virtual override returns (address) {
        ...
        liquidityPool = address(new PoolMock(IVault(address(vault)), "ERC20 Pool", "ERC20POOL"));
        vm.label(address(liquidityPool), "liquidityPool");

        factoryMock.registerTestPool(address(liquidityPool), tokenConfig);

        swapPool = address(new PoolMock(IVault(address(vault)), "ERC20 Pool", "ERC20POOL"));
        vm.label(address(swapPool), "swapPool");

        factoryMock.registerTestPool(address(swapPool), tokenConfig);
    ...
    // Manually setting the swap fee bypasses the minimum swap fee check.
    function setSwapFees(uint256 swapFeePercentage) internal {
        if (swapFeePercentage < MIN_SWAP_FEE) {
            vault.manuallySetSwapFee(liquidityPool, swapFeePercentage);
            vault.manuallySetSwapFee(swapPool, swapFeePercentage);
        } else {
