...
contract CompositeLiquidityRouterNestedPoolsTest is BaseERC4626BufferTest {
    ...
    address internal parentPoolDupToken;
    ...
    function setUp() public override {
        ...
        (parentPool, ) = _createPool(
            [address(childPoolA), address(childPoolB), address(usdt)].toMemoryArray(),
            "parentPool"
        );
        (parentPoolDupToken, ) = _createPool(
        ...
        approveForPool(IERC20(parentPoolDupToken));
        ...
        _initPoolUnsorted(
            parentPool,
            [childPoolA, childPoolB, address(usdt)].toMemoryArray(),
    ...
    function _initPoolUnsorted(
        ...
        return _initPool(poolToInitialize, sortedAmountsIn, 0);
    }
    ...
    *******************************************************************************/
    ...
    function testAddLiquidityNestedPool__Fuzz(uint256[5] memory amounts) public {
        uint256 totalAmountIn;

        for (uint256 i = 0; i < 5; ++i) {
            amounts[i] = bound(amounts[i], PRODUCTION_MIN_TRADE_AMOUNT, 10 * poolInitAmount);
        ...
        tokensIn[vars.usdtIdx] = address(usdt);

        uint256[] memory amountsIn = new uint256[](5);
        amountsIn[vars.daiIdx] = amounts[vars.daiIdx];
        amountsIn[vars.usdcIdx] = amounts[vars.usdcIdx];
        amountsIn[vars.wethIdx] = amounts[vars.wethIdx];
        amountsIn[vars.wstethIdx] = amounts[vars.wstethIdx];
        amountsIn[vars.usdtIdx] = amounts[vars.usdtIdx];
        ...
        uint256 expectedBptOut = totalAmountIn;
        ...
        assertEq(vars.lpAfter.usdt, vars.lpBefore.usdt - amountsIn[vars.usdtIdx], "LP Usdt Balance is wrong");
        ...
        assertEq(vars.vaultAfter.usdt, vars.vaultBefore.usdt + amountsIn[vars.usdtIdx], "Vault Usdt Balance is wrong");
        ...
        assertApproxEqAbs(
            vars.parentPoolAfter.usdt,
            vars.parentPoolBefore.usdt + amountsIn[vars.usdtIdx],
            MAX_ROUND_ERROR,
            "ParentPool Usdt Balance is wrong"
        );
        assertEq(
    ...
    function testAddLiquidityDuplicateNestedPoolToken() public {
        ...
        compositeLiquidityRouter.addLiquidityUnbalancedNestedPool(
            parentPoolDupToken,
            [address(dai), address(usdc), address(weth), address(wsteth)].toMemoryArray(),
            amountsIn,
            new address[](0),
            0,
            false,
    ...
    function testAddLiquidityUnbalanceToOneChildNestedPool() public {
        ...
        // Check ParentPool balances.
        assertEq(vars.parentPoolAfter.usdt, vars.parentPoolBefore.usdt, "ParentPool Usdt Balance is wrong");
        assertEq(
            vars.parentPoolAfter.childPoolABpt,
            vars.parentPoolBefore.childPoolABpt + mintedChildPoolABpt,
            "ParentPool ChildPoolA BPT Balance is wrong"
    ...
    function testAddLiquidityNestedPoolMissingToken() public {
        ...
        assertEq(
            vars.childPoolBAfter.wsteth,
            vars.childPoolBBefore.wsteth, // Wsteth not added
            "ChildPoolB Wsteth Balance is wrong"
        );
        ...
        assertEq(
            vars.parentPoolAfter.usdt,
            vars.parentPoolBefore.usdt, // usdt not added
            "ParentPool Usdt Balance is wrong"
        );
        assertEq(
            vars.parentPoolAfter.childPoolABpt,
    ...
    function testAddLiquidityNestedPoolDuplicateInputTokens() public {
        uint256 daiAmount = poolInitAmount;
        uint256 wethAmount = poolInitAmount;
    ...
    *******************************************************************************/
    ...
    function testRemoveLiquidityNestedPool__Fuzz(
        uint256 proportionToRemove,
        bool parentRecovery,
        bool poolARecovery,
        bool poolBRecovery
    ) public {
        ...
        if (parentRecovery) {
            vault.manualEnableRecoveryMode(parentPool);
        }
        if (poolARecovery) {
            vault.manualEnableRecoveryMode(childPoolA);
        }
        if (poolBRecovery) {
        ...
        address[] memory tokensOut = new address[](5);
        ...
        tokensOut[vars.usdtIdx] = address(usdt);
        ...
        uint256[] memory amountsOut = _removeLiquidityProportionalNestedPool(
            ...
            exactBptIn,
            tokensOut,
            minAmountsOut, // Zero minimums - no intermediate checks
            new address[](0),
            false,
            bytes("")
        );
        ...
        // Direct tokens withdrawn from parent pool are precise.
        uint256 expectedParentOut = poolInitAmount.mulDown(proportionToRemove);
        // Nested tokens must take rounding and reserved BPT into account.
        uint256 expectedNestedOut = poolInitAmount.mulDown(proportionToRemove) - deadTokens - MAX_ROUND_ERROR;

        uint256[] memory expectedAmountsOut = new uint256[](5);

        expectedAmountsOut[vars.daiIdx] = expectedNestedOut;
        expectedAmountsOut[vars.wethIdx] = expectedNestedOut;
        expectedAmountsOut[vars.wstethIdx] = expectedNestedOut;
        expectedAmountsOut[vars.usdcIdx] = expectedNestedOut;
        expectedAmountsOut[vars.usdtIdx] = expectedParentOut;

        // Check returned token amounts.
        assertEq(amountsOut.length, 5, "amountsOut length is wrong");

        // The composite router has no Recovery Mode handling, so the parent and both children take the ordinary
        // path whatever these flags are set to, and the same tolerance holds for all eight combinations. Every
        // token here is 18-decimal, STANDARD and yield-fee exempt, so the flags do not move the amounts either.
        ...
        assertApproxEqAbs(
            expectedAmountsOut[vars.usdtIdx],
            amountsOut[vars.usdtIdx],
            tolerance,
            "USDT amount out is wrong"
        );
        ...
        assertEq(vars.vaultAfter.usdt, vars.vaultBefore.usdt - amountsOut[vars.usdtIdx], "Vault Usdt Balance is wrong");
        ...
        assertEq(
            vars.vaultAfter.childPoolABpt,
            vars.vaultBefore.childPoolABpt - vars.burnedChildPoolABpt,
            "Vault ChildPoolA BPT Balance is wrong"
        );
        assertEq(
            vars.vaultAfter.childPoolBBpt,
            vars.vaultBefore.childPoolBBpt - vars.burnedChildPoolBBpt,
            "Vault ChildPoolB BPT Balance is wrong"
        );
        ...
        assertApproxEqAbs(
            vars.childPoolBAfter.dai,
            vars.childPoolBBefore.dai - amountsOut[vars.daiIdx],
            MAX_ROUND_ERROR,
            "ChildPoolB Dai Balance is wrong"
        );
        assertEq(
        ...
        assertApproxEqAbs(
            vars.parentPoolAfter.usdt,
            vars.parentPoolBefore.usdt - amountsOut[vars.usdtIdx],
            MAX_ROUND_ERROR,
            "ParentPool Usdt Balance is wrong"
        );
        assertEq(
            vars.parentPoolAfter.childPoolABpt,
            vars.parentPoolBefore.childPoolABpt - vars.burnedChildPoolABpt,
            "ParentPool ChildPoolA BPT Balance is wrong"
        );
        assertEq(
            vars.parentPoolAfter.childPoolBBpt,
            vars.parentPoolBefore.childPoolBBpt - vars.burnedChildPoolBBpt,
            "ParentPool ChildPoolB BPT Balance is wrong"
        );
    }
    ...
    ***************************************************************************/
    ...
    // Where the parent pool is paused, the composite call reverts and the caller unwinds one level at a time:
    // `Router.removeLiquidityRecovery` on the parent returns its registered tokens, child BPT among them as a plain
    // ERC20, and each child is then removed on its own.

    function testRemoveLiquidityNestedPoolPausedParentReverts() public {
        uint256 exactBptIn = BalancerPoolToken(parentPool).totalSupply() / 10;

        vault.manualSetPoolPaused(parentPool, true);
        vault.manualEnableRecoveryMode(parentPool);

        address[] memory tokensOut = new address[](5);
        tokensOut[0] = address(dai);
        tokensOut[1] = address(weth);
        tokensOut[2] = address(wsteth);
        tokensOut[3] = address(usdc);
        tokensOut[4] = address(usdt);

        // `startPrank` rather than `prank`: the prepaid variant of this helper approves the BPT first, and a
        // single-call prank would be spent on that approval rather than on the withdrawal.
        vm.startPrank(lp);
        _removeLiquidityProportionalNestedPool(
            ...
            false,
            bytes(""),
            abi.encodeWithSelector(IVaultErrors.PoolPaused.selector, parentPool)
        );
        vm.stopPrank();

        // The query reverts with the same error.
        _prankStaticCall();
        vm.expectRevert(abi.encodeWithSelector(IVaultErrors.PoolPaused.selector, parentPool));
    ...
    function testRemoveLiquidityNestedPoolPausedParentUnwindsInSteps() public {
        uint256 exactBptIn = BalancerPoolToken(parentPool).totalSupply() / 10;

        vault.manualSetPoolPaused(parentPool, true);
        vault.manualEnableRecoveryMode(parentPool);

        IERC20[] memory parentTokens = vault.getPoolTokens(parentPool);

        uint256 childABptBefore = IERC20(childPoolA).balanceOf(lp);
        uint256 usdcBefore = usdc.balanceOf(lp);
        uint256 wethBefore = weth.balanceOf(lp);

        // Step 1: the parent's registered tokens, with child BPT delivered as an ordinary ERC20.
        vm.prank(lp);
        uint256[] memory parentAmountsOut = router.removeLiquidityRecovery(
            parentPool,
            exactBptIn,
            new uint256[](parentTokens.length)
        );

        uint256 childABptOut;
        for (uint256 i = 0; i < parentTokens.length; ++i) {
            if (address(parentTokens[i]) == childPoolA) {
                childABptOut = parentAmountsOut[i];
            }
        }
        ...
        // Step 2: the child is unpaused and not in Recovery Mode, so the ordinary path unwinds it.
        assertFalse(vault.isPoolInRecoveryMode(childPoolA), "childPoolA is in Recovery Mode");

        vm.prank(lp);
        uint256[] memory childAmountsOut = router.removeLiquidityProportional(
            childPoolA,
            childABptOut,
            new uint256[](2),
            false,
        ...
        assertGt(childAmountsOut[0], 0, "First childPoolA token paid nothing");
        assertGt(childAmountsOut[1], 0, "Second childPoolA token paid nothing");
        assertGt(usdc.balanceOf(lp), usdcBefore, "LP: no USDC recovered");
        assertGt(weth.balanceOf(lp), wethBefore, "LP: no WETH recovered");
    }
    ...
    function testRemoveLiquidityNestedPoolLimits() public {
        ...
        minAmountsOut[vars.usdtIdx] = poolInitAmount;
        ...
        uint256 daiExpectedAmountOut = poolInitAmount.mulDown(proportionToRemove) - deadTokens;
    ...
    function testRemoveLiquidityNestedPoolWrongTokenArray() public {
        ...
        address[] memory actualTokensOut = new address[](5);
        actualTokensOut[0] = address(usdt);
        actualTokensOut[1] = address(dai);
        actualTokensOut[2] = address(wsteth);
        actualTokensOut[3] = address(weth);
        actualTokensOut[4] = address(usdc);
    ...
    *******************************************************************************/
    ...
    // The Vault rejects a zero wrap amount, so the router skips the buffer call instead of handing one over. These
    // tests use a 6-decimal wrapper, whose raw proportional output rounds to zero at burns that still pay the
    // 18-decimal tokens in full. The pools are built on demand, since nothing else uses them.

    // Burn 1/1e7 of the supply: the waUSDC6 token holds 1e6 raw, so its proportional output rounds to zero, while
    // the 18-decimal tokens (1e6 * 1e18 raw) still return 1e17.
    uint256 private constant _ZERO_SHARE_BURN_DIVISOR = 1e7;
    // Burn 1/1e3 of the supply, which puts the waUSDC6 share well above the buffer minimum. Used as the control.
    uint256 private constant _NON_ZERO_SHARE_BURN_DIVISOR = 1e3;

    uint256 private constant _WA_USDC6_POOL_BALANCE = 1e6; // 1.0 unit of a 6-decimal wrapper
    uint256 private constant _WA18_POOL_BALANCE = 1e6 * 1e18;
    uint256 private constant _WA_USDC6_BUFFER_UNDERLYING = 1e5 * 1e6;

    // Plain USDC-6 balance for the pool that also holds it as the wrapper's underlying.
    uint256 private constant _USDC6_POOL_BALANCE = 1e12;

    ERC4626TestToken private _waUSDC6;
    // [waUSDC6, waDAI]: the zero share is a parent pool token.
    address private _zeroShareParentPool;
    // [waUSDC6, waWETH], nested in [_zeroShareChildPool, waDAI]: the zero share is a child pool token.
    address private _zeroShareChildPool;
    address private _zeroShareNestedPool;
    // [waUSDC6, usdc6Decimals, waDAI]: the zero share's underlying is also a plain pool token.
    ...
    function testRemoveLiquidityNestedPoolZeroUnwrapParentToken() public {
        ...
        // The zero share reports zero and transfers nothing.
    ...
    function testRemoveLiquidityNestedPoolZeroUnwrapTokenIsRegistered() public {
        ...
        (uint256 waUsdc6Idx, uint256 waDaiIdx_) = getSortedIndexes(address(_waUSDC6), address(waDAI));
        address[] memory actualTokensOut = new address[](2);
        actualTokensOut[waUsdc6Idx] = address(usdc6Decimals);
        actualTokensOut[waDaiIdx_] = address(dai);

        // The zero share produces no output, but it is still an output token, so omitting it fails.
        ...
        _removeLiquidityProportionalNestedPool(
            ...
            bytes(""),
            abi.encodeWithSelector(
                ICompositeLiquidityRouterErrors.WrongTokensOut.selector,
                actualTokensOut,
                shortTokensOut
            )
        );
    ...
    function testRemoveLiquidityNestedPoolZeroUnwrapNonZeroLimit() public {
        ...
        // A limit on the zero share fails with the limit error, not the buffer's. The flat ERC4626 path does the
        // same.
        vm.startPrank(lp);
        _removeLiquidityProportionalNestedPool(
            _zeroShareParentPool,
            ...
            false,
            bytes(""),
            abi.encodeWithSelector(IVaultErrors.AmountOutBelowMin.selector, address(usdc6Decimals), 0, 1)
        );
        vm.stopPrank();
    }
    ...
    function testRemoveLiquidityNestedPoolNonZeroUnwrapUnchanged() public {
        ...
        // Control: an amount comfortably above the buffer minimum.
    ...
    function testRemoveLiquidityNestedPoolZeroUnwrapUnderlyingAlsoPlainToken() public {
        ...
        // The waUSDC6 token is unwrapped into USDC-6, which the pool also holds directly, so the traversal registers
        ...
        // What the pool itself returns for this burn, in pool registration order.
        uint256 snapshotId = vm.snapshotState();
        _prankStaticCall();
        ...
        uint256[] memory poolTokenIndexes = getSortedIndexes(
            [address(_waUSDC6), address(usdc6Decimals), address(waDAI)].toMemoryArray()
        );
        assertEq(rawAmountsOut[poolTokenIndexes[0]], 0, "Setup: waUSDC6 share should be exactly zero");
        ...
        // Registering the zero adds nothing to what the plain token already wrote, and does not enlarge the set.
        assertEq(
            amountsOut[usdc6Idx],
        ...
        returns (address[] memory tokensOut, address[] memory tokensToUnwrap)
    {
        ...
        tokensOut[0] = address(usdc6Decimals);
        tokensOut[1] = address(weth);
        tokensOut[2] = address(dai);

        tokensToUnwrap = new address[](3);
        tokensToUnwrap[0] = address(_waUSDC6);
        tokensToUnwrap[1] = address(waWETH);
    ...
    function _createZeroSharePools() private {
        ...
        (_zeroShareChildPool, ) = _createPool(
            [address(_waUSDC6), address(waWETH)].toMemoryArray(),
            "zeroShareChildPool"
        );
        (_zeroShareParentPool, ) = _createPool(
            [address(_waUSDC6), address(waDAI)].toMemoryArray(),
            "zeroShareParentPool"
        );
        ...
        vm.startPrank(lp);
        _waUSDC6.deposit(_WA_USDC6_BUFFER_UNDERLYING + 3 * _WA_USDC6_POOL_BALANCE, lp);
        ...
        _initPoolUnsorted(
            _zeroShareParentPool,
            [address(_waUSDC6), address(waDAI)].toMemoryArray(),
            [_WA_USDC6_POOL_BALANCE, _WA18_POOL_BALANCE].toMemoryArray()
        );
        ...
        _initPoolUnsorted(
            _zeroShareDupUnderlyingPool,
            [address(_waUSDC6), address(usdc6Decimals), address(waDAI)].toMemoryArray(),
            [_WA_USDC6_POOL_BALANCE, _USDC6_POOL_BALANCE, _WA18_POOL_BALANCE].toMemoryArray()
        );
        vm.stopPrank();
    }
    ...
    struct NestedPoolTestLocals {
        ...
        uint256 waWethIdx;
        uint256 waUsdcIdx;
        uint256 burnedChildPoolABpt;
        uint256 burnedChildPoolBBpt;
    ...
    function _createNestedPoolTestLocals() private view returns (NestedPoolTestLocals memory vars) {
        ...
        (vars.daiIdx, vars.wethIdx, vars.wstethIdx, vars.usdcIdx, vars.usdtIdx) = (0, 1, 2, 3, 4);
    ...
    function _getBalances(address entity) private view returns (TokenBalances memory balances) {
        ...
        balances.usdt = usdt.balanceOf(entity);
