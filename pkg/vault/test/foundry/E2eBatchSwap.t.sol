...
contract E2eBatchSwapTest is BaseVaultTest {
    ...
    ERC20TestToken internal token3;
    ERC20TestToken internal token4;

    IERC20[] internal tokensToTrack;

    uint256 internal idxToken1;
    uint256 internal idxToken2;
    uint256 internal idxToken3;
    uint256 internal idxToken4;
    ...
    uint256 internal minSwapAmountToken4;
    uint256 internal maxSwapAmountToken4;
    ...
    function setUp() public virtual override {
        ...
        // Create pool1
        vm.startPrank(lp);
        pool1 = _createPool([address(token1), address(token2)].toMemoryArray(), "Pool1");
        _initPool(pool1, [poolInitAmount, poolInitAmount].toMemoryArray(), 0);
        // Create pool2
        pool2 = _createPool([address(token2), address(token3)].toMemoryArray(), "Pool2");
        _initPool(pool2, [poolInitAmount, poolInitAmount].toMemoryArray(), 0);
        // Create pool3
        pool3 = _createPool([address(token3), address(token4)].toMemoryArray(), "Pool3");
        _initPool(pool3, [poolInitAmount, poolInitAmount].toMemoryArray(), 0);
        vm.stopPrank();

        tokensToTrack = [address(token1), address(token2), address(token3), address(token4)].toMemoryArray().asIERC20();
    ...
    function _setUpVariables() internal virtual {
        token1 = dai;
        token2 = usdc;
        token3 = ERC20TestToken(address(weth));
        token4 = wsteth;
        sender = lp;
        poolCreator = lp;
        ...
        minSwapAmountToken4 = 10 * MIN_TRADE_AMOUNT;
        maxSwapAmountToken4 = poolInitAmount;
    ...
    function testDoExactInUndoExactInNoFees__Fuzz(uint256 exactAmountIn) public {
        ...
        IBatchRouter.SwapPathExactAmountIn[] memory swapPath = _buildExactInPaths(
            IERC20(address(token1)),
            exactAmountIn,
            0
        );
        (uint256[] memory pathAmountsOutDo, , ) = batchRouter.swapExactIn(swapPath, MAX_UINT128, false, bytes(""));

        IBatchRouter.SwapPathExactAmountIn[] memory reverseSwapPath = _buildExactInPaths(
            IERC20(address(token4)),
            pathAmountsOutDo[0],
            0
        );
        (uint256[] memory pathAmountsOutUndo, , ) = batchRouter.swapExactIn(
            reverseSwapPath,
            MAX_UINT128,
            false,
            bytes("")
        ...
        assertLe(pathAmountsOutUndo[0], exactAmountIn, "Amount out undo should be <= exactAmountIn");
        ...
        assertEq(
            balancesAfter.userTokens[idxToken3],
            balancesBefore.userTokens[idxToken3],
            "Wrong sender token3 balance"
        );
        assertLe(
            balancesAfter.userTokens[idxToken4],
            balancesBefore.userTokens[idxToken4],
            "Wrong sender token4 balance"
        );
    }
    ...
    function _getSwapSteps(IERC20 tokenIn) private view returns (IBatchRouter.SwapPathStep[] memory steps) {
        ...
        if (address(tokenIn) == address(token4)) {
            steps[0] = IBatchRouter.SwapPathStep({ pool: pool3, tokenOut: IERC20(address(token3)), isBuffer: false });
            steps[1] = IBatchRouter.SwapPathStep({ pool: pool2, tokenOut: IERC20(address(token2)), isBuffer: false });
            steps[2] = IBatchRouter.SwapPathStep({ pool: pool1, tokenOut: IERC20(address(token1)), isBuffer: false });
        } else {
            steps[0] = IBatchRouter.SwapPathStep({ pool: pool1, tokenOut: IERC20(address(token2)), isBuffer: false });
            steps[1] = IBatchRouter.SwapPathStep({ pool: pool2, tokenOut: IERC20(address(token3)), isBuffer: false });
            steps[2] = IBatchRouter.SwapPathStep({ pool: pool3, tokenOut: IERC20(address(token4)), isBuffer: false });
        }
    }
}
