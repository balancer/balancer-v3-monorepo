...
contract BaseBatchRouterE2ETest is BaseVaultTest {
    ...
    uint256[] private _expectedPathAmountsOut;
    EnumerableMap.AddressToUintMap private _expectedAmountsOut;
    ...
    ***************************************************************************/
    ...
    function generateSimpleDiffs(SwapPathExactAmountIn[] memory pathsExactIn) internal {
        for (uint256 i = 0; i < pathsExactIn.length; i++) {
            ...
            addAmountOut(lastStep.tokenOut, pathExactIn.minAmountOut);
            addPathAmountOut(pathExactIn.minAmountOut);
        }
    }

    // This line is moved into a separate method to provide encapsulation in case we need to extend it later.
    function addPathAmountOut(uint256 amount) internal {
        _expectedPathAmountsOut.push(amount);
    }

    // This line is moved into a separate method to provide encapsulation in case we need to extend it later.
    function addAmountOut(IERC20 token, uint256 amount) internal {
        (, uint256 currentAmountOut) = _expectedAmountsOut.tryGet(address(token));
        _expectedAmountsOut.set(address(token), currentAmountOut + amount);
    }
    ...
    function testSwapExactIn(
        ...
        _assertApproxEqAbsArrays(
            calculatedPathAmountsOut,
            _expectedPathAmountsOut,
            delta,
            "Expected path amounts out different than actual"
        );
        _assertEqArrays(
            queryCalculatedPathAmountsOut,
            calculatedPathAmountsOut,
            "Query path amounts out different than actual"
        );

        uint256[] memory expectedAmountsOut = new uint256[](_expectedAmountsOut.length());
        for (uint256 i = 0; i < expectedAmountsOut.length; i++) {
            (, expectedAmountsOut[i]) = _expectedAmountsOut.at(i);
        }
