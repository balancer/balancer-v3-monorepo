...
contract Vault is IVault, Authentication, ERC20MultiToken, ReentrancyGuard {
    ...
    *******************************************************************************/
        ...
        returns (uint256 amountCalculated, uint256 amountIn, uint256 amountOut)
    {
        ...
        uint256 amountGivenScaled18 = params.kind == SwapKind.GIVEN_IN
        ...
        if (vars.swapFeePercentage > 0 && params.kind == IVault.SwapKind.GIVEN_OUT) {
            ...
            vars.swapFeeAmount = amountGivenScaled18.divUp(vars.swapFeePercentage.complement()) - amountGivenScaled18;
        }

        // Add swap fee to the amountGiven to account for the fee taken in GIVEN_OUT swap on tokenOut
        // Perform the swap request callback and compute the new balances for 'token in' and 'token out' after the swap
        uint256 amountCalculatedScaled18 = IBasePool(params.pool).onSwap(
            IBasePool.SwapParams({
                kind: params.kind,
                tokenIn: params.tokenIn,
                tokenOut: params.tokenOut,
                amountGivenScaled18: amountGivenScaled18 + vars.swapFeeAmount,
        ...
        if (vars.swapFeePercentage > 0 && params.kind == IVault.SwapKind.GIVEN_IN) {
            ...
            // Round up to avoid losses during precision loss.
            vars.swapFeeAmount = amountCalculatedScaled18.mulUp(vars.swapFeePercentage);
            // Should substract the fee from the amountCalculated for GIVEN_IN swap
            amountCalculatedScaled18 -= vars.swapFeeAmount;
        }
        ...
        amountCalculated = params.kind == SwapKind.GIVEN_IN
            ? amountCalculatedScaled18.toRawRoundDown(vars.scalingFactors[vars.indexOut])
            : amountCalculatedScaled18.toRawRoundUp(vars.scalingFactors[vars.indexIn]);
        ...
        if (vars.config.callbacks.shouldCallAfterSwap) {
            (uint256 amountInScaled18, uint256 amountOutScaled18) = params.kind == SwapKind.GIVEN_IN
                ? (amountGivenScaled18, amountCalculatedScaled18)
                : (amountCalculatedScaled18, amountGivenScaled18);
