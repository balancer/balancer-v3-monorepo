...
library HooksConfigLib {
    ...
    function isAnyInitializeHookEnabled(PoolConfigBits config) internal pure returns (bool) {
        return PoolConfigBits.unwrap(config).decodeBool(PoolConfigConst.ANY_INITIALIZE_HOOK_ENABLED_OFFSET);
    }

    function setAnyInitializeHookEnabled(PoolConfigBits config, bool value) internal pure returns (PoolConfigBits) {
        return
            PoolConfigBits.wrap(
                PoolConfigBits.unwrap(config).insertBool(value, PoolConfigConst.ANY_INITIALIZE_HOOK_ENABLED_OFFSET)
            );
    }

    function isAnySwapHookEnabled(PoolConfigBits config) internal pure returns (bool) {
        return PoolConfigBits.unwrap(config).decodeBool(PoolConfigConst.ANY_SWAP_HOOK_ENABLED_OFFSET);
    }

    function setAnySwapHookEnabled(PoolConfigBits config, bool value) internal pure returns (PoolConfigBits) {
        return
            PoolConfigBits.wrap(
                PoolConfigBits.unwrap(config).insertBool(value, PoolConfigConst.ANY_SWAP_HOOK_ENABLED_OFFSET)
            );
    }

    function isAnyAddLiquidityHookEnabled(PoolConfigBits config) internal pure returns (bool) {
        return PoolConfigBits.unwrap(config).decodeBool(PoolConfigConst.ANY_ADD_LIQUIDITY_HOOK_ENABLED_OFFSET);
    }

    function setAnyAddLiquidityHookEnabled(PoolConfigBits config, bool value) internal pure returns (PoolConfigBits) {
        return
            PoolConfigBits.wrap(
                PoolConfigBits.unwrap(config).insertBool(value, PoolConfigConst.ANY_ADD_LIQUIDITY_HOOK_ENABLED_OFFSET)
            );
    }

    function isAnyRemoveLiquidityHookEnabled(PoolConfigBits config) internal pure returns (bool) {
        return PoolConfigBits.unwrap(config).decodeBool(PoolConfigConst.ANY_REMOVE_LIQUIDITY_HOOK_ENABLED_OFFSET);
    }
        ...
        return
            PoolConfigBits.wrap(
                PoolConfigBits.unwrap(config).insertBool(
                    value,
                    PoolConfigConst.ANY_REMOVE_LIQUIDITY_HOOK_ENABLED_OFFSET
                )
            );
    }
