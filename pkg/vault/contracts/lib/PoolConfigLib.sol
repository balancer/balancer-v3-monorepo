...
library PoolConfigLib {
    ...
    uint256 public constant PROTOCOL_SWAP_FEE_OFFSET = STATIC_SWAP_FEE_OFFSET + FEE_BITLENGTH;
    uint256 public constant PROTOCOL_YIELD_FEE_OFFSET = PROTOCOL_SWAP_FEE_OFFSET + FEE_BITLENGTH;
    uint256 public constant DECIMAL_SCALING_FACTORS_OFFSET = PROTOCOL_YIELD_FEE_OFFSET + FEE_BITLENGTH;
    ...
    function fromPoolConfig(PoolConfig memory config) internal pure returns (PoolConfigBits) {
        ...
        }
        {
            configBits = configBits
                ...
                .insertUint(
                    config.aggregateSwapFeePercentage / FEE_SCALING_FACTOR,
                    PROTOCOL_SWAP_FEE_OFFSET,
                    FEE_BITLENGTH
                )
                .insertUint(
                    config.aggregateYieldFeePercentage / FEE_SCALING_FACTOR,
        ...
        return
            PoolConfig({
                ...
                staticSwapFeePercentage: rawConfig.decodeUint(STATIC_SWAP_FEE_OFFSET, FEE_BITLENGTH) *
                    FEE_SCALING_FACTOR,
                aggregateSwapFeePercentage: rawConfig.decodeUint(PROTOCOL_SWAP_FEE_OFFSET, FEE_BITLENGTH) *
                    FEE_SCALING_FACTOR,
                aggregateYieldFeePercentage: rawConfig.decodeUint(PROTOCOL_YIELD_FEE_OFFSET, FEE_BITLENGTH) *
                    FEE_SCALING_FACTOR,
